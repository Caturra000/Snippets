...
test_success() {
    ...
    pushd "$dir" > /dev/null
    if bash test.sh "$(dirs -l +1)/$LINKER" > test_output.log 2>&1; then
        echo -e "${GREEN}PASS${NC}"
        ((PASS++))
    else
        echo -e "${RED}FAIL${NC}"
        cat test_output.log | head -10 | sed 's/^/    /'
        ((FAIL++))
    fi
    popd > /dev/null
}
...
test_failure "11 Undefined symbol error"   "$TESTS_DIR/11_undef_error"
test_failure "12 Duplicate symbol error"   "$TESTS_DIR/12_dup_error"
