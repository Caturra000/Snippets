// 基于原有的catmix.cpp，改进插桩例程的开销
// 为了方便，这里不区分 predicated 和 unpredicated
...
const UINT32 MAX_INDEX = XED_CATEGORY_LAST;
...
{
  public:
    BBLSTATS(UINT32* stats) : _stats(stats), _counter(0) {};
...
VOID ComputeGlobalStats()
{
    for(UINT32 i = 0; i < MAX_INDEX; i++)
    {
        ...
        {
            GlobalStatsDynamic.unpredicated[i] += (*bi)->_counter * (*bi)->_stats[i];
        }
    }
}
...
VOID Trace(TRACE trace, VOID* v)
{
    for (BBL bbl = TRACE_BblHead(trace); BBL_Valid(bbl); bbl = BBL_Next(bbl))
    {
        ...
        UINT32* stats = new UINT32[MAX_INDEX];

        for (INS ins = BBL_InsHead(bbl); INS_Valid(ins); ins = INS_Next(ins))
        {
            stats[INS_Category(ins)]++;
        }
